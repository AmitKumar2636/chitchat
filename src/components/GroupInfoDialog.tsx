...
export function GroupInfoDialog(props: Props) {
  ...
  async function handleRename(e: Event) {
    ...
    try {
      ...
      // Optional: show success or just update local state if needed (but chat prop should update)
    } catch (err) {
  ...
  async function handleAddMember(e: Event) {
    ...
    // Check if already in group (by email if possible, but we only have UIDs in participants)
    // We'd need to map email to UID first.
          ...
          <Dialog.Content class="bg-white dark:bg-wa-dark-sidebar rounded-lg p-6 w-full max-w-md shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <div class="flex flex-col gap-6">
              ...
              <div class="flex gap-2 items-end">
                <TextField value={groupName()} onChange={setGroupName} class="flex-1">
                    <TextField.Label class="text-sm text-wa-text-secondary dark:text-wa-dark-text-secondary mb-1 block">
                      Group Name
                    </TextField.Label>
                    <TextField.Input
                      class="w-full px-3 py-2 rounded-lg border border-wa-border dark:border-wa-dark-border bg-wa-header dark:bg-wa-dark-header text-wa-text-primary dark:text-wa-dark-text-primary focus:outline-none focus:border-wa-teal"
                    />
                </TextField>
                <Button
